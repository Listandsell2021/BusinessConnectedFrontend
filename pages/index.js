...
export default function Home() {
  ...
  const [formData, setFormData] = useState({
    email: '',
    password: '',
    selectedService: '' // Multi-service partner dashboard
  });
...
}, [isGerman, formData.email, formData.password, formData.selectedService, errors, lastErrorResult]);
...
const handleSubmit = async (e) => {
  ...
  try {
    // Login with security service only
    const result = await login(formData.email, formData.password, 'security');

    if (result && result.success) {
      // Set the service to security
      switchService('security');
  ...
  return (
    <>
      <Head>
        <title>{isGerman ? 'Partner-Anmeldung' : 'Partner Login'} - Business Connected</title>
        <meta name="description" content={isGerman ? 'Partner-Anmeldung für Business Connected' : 'Partner login for Business Connected'} />
      </Head>
        ...
        <div className="hidden md:block relative w-0 flex-1">
          ...
          >
            ...
            <div className="text-center p-6 sm:p-8 max-w-lg relative z-10">
              ...
              >
                {isGerman
                  ? 'Verwalten Sie Ihre Leads professionell mit unserem CRM-System.'
                  : 'Manage your leads professionally with our CRM system.'
                }
              </motion.p>
              ...
              <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-10">
                {[
                  {
                    icon: '📋',
                    title: isGerman ? 'Leads verwalten' : 'Manage Leads',
                    desc: isGerman ? 'Professionelle Verwaltung' : 'Professional management'
                  },
                  {
                    icon: '🔔',
              ...
              >
                <div className="flex items-center justify-center space-x-2">
                  ...
                  <span className="text-sm font-medium" style={{ color: 'var(--theme-text)' }}>
                    {isGerman ? 'Sicher & Geprüft' : 'Secure & Verified'}
                  </span>
                </div>
              </motion.div>
            </div>
