...
export default function PartnerRequest() {
  ...
  useEffect(() => {
    if (Object.keys(errors).length > 0) {
      ...
      if (formData.regions.length === 0) {
        newErrors.regions = isGerman ? 'Bitte wählen Sie mindestens eine Region aus' : 'Please select at least one region';
      }
      ...
      if (!formData.periodOfAvailability) {
        newErrors.periodOfAvailability = isGerman ? 'Verfügbarkeitszeitraum ist erforderlich' : 'Availability period is required';
      }

      // Budget Scope validation
      if (formData.budgetScope.length === 0) {
        newErrors.budgetScope = isGerman ? 'Bitte wählen Sie mindestens einen Service aus' : 'Please select at least one service';
      }
  ...
  }, [isGerman, formData.companyName, formData.contactPerson, formData.email, formData.phone, formData.regions, formData.availableEmployees, formData.periodOfAvailability, formData.budgetScope, formData.companyDescription, formData.agreeToTerms, errors]);
  ...
  const getFirstError = () => {
    const errorKeys = [
      'companyName', 'contactPerson', 'email', 'phone', 'regions', 'availableEmployees', 'periodOfAvailability', 'budgetScope', 'companyDescription', 'agreeToTerms'
    ];
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/auth/register-partner`, {
        ...
        body: JSON.stringify({
          ...
          // Password will be auto-generated by server
          services: ['security'], // Only security service supported
          partnerType: 'basic',
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        className="w-full px-3 py-3 sm:px-4 sm:py-4 border-2 border-gray-300 dark:border-gray-200/30 rounded-xl bg-white dark:bg-gray-800"
                        style={{
                          color: 'var(--theme-text)',
                          backgroundColor: 'var(--theme-bg-secondary, rgba(0, 0, 0, 0.05))'
                        }}
                  ...
                  >
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      rows="6"
                      className="w-full px-3 py-3 sm:px-4 sm:py-4 border-2 border-gray-300 dark:border-gray-200/30 rounded-xl"
                      style={{
                        backgroundColor: 'var(--theme-bg-secondary, rgba(0, 0, 0, 0.05))',
                        color: 'var(--theme-text)',
                        backdropFilter: 'blur(10px)'
                        ...
                        {isGerman ? (
                          <>
                            ...
                            <div>
                              ...
                              <p>Wenn Besucher Kommentare oder Formulare auf der Website ausfüllen, sammeln wir die Daten, die im Formular angezeigt werden, außerdem die IP-Adresse des Besuchers und den User-Agent-String (damit wird der Browser identifiziert), um die Erkennung von Spam zu unterstützen.</p>
                            ...
                            <div>
                              ...
                              <p className="mt-2">Falls du ein Konto hast und dich auf dieser Website anmeldest, werden wir ein temporäres Cookie setzen, um festzustellen, ob dein Browser Cookies akzeptiert. Dieses Cookie enthält keine personenbezogenen Daten und wird verworfen, wenn du deinen Browser schließt.</p>
                              <p className="mt-2">Wenn du dich anmeldest, werden wir einige Cookies einrichten, um deine Anmeldeinformationen und Anzeigeoptionen zu speichern. Anmelde-Cookies verfallen nach zwei Tagen und Cookies für die Anzeigeoptionen nach einem Jahr. Falls du bei der Anmeldung &ldquo;Angemeldet bleiben&rdquo; auswählst, wird deine Anmeldung zwei Wochen lang aufrechterhalten. Mit der Abmeldung aus deinem Konto werden die Anmelde-Cookies gelöscht.</p>
                              <p className="mt-2">Wenn du einen Artikel bearbeitest oder veröffentlichst, wird ein zusätzlicher Cookie in deinem Browser gespeichert. Dieser Cookie enthält keine personenbezogenen Daten und verweist nur auf die Beitrags-ID des Artikels, den du gerade bearbeitet hast. Der Cookie verfällt nach einem Tag.</p>
                            </div>
                            ...
                            <div>
                              ...
                              <p className="mt-2">Diese Websites können Daten über dich sammeln, Cookies benutzen, zusätzliche Tracking-Dienste von Dritten einbetten und deine Interaktion mit diesem eingebetteten Inhalt aufzeichnen, inklusive deiner Interaktion mit dem eingebetteten Inhalt, falls du ein Konto hast und auf dieser Website angemeldet bist.</p>
                            </div>
                            ...
                            <div>
                              ...
                              <p className="mt-2">Für Benutzer, die sich auf unserer Website registrieren, speichern wir zusätzlich die persönlichen Informationen, die sie in ihren Benutzerprofilen angeben. Alle Benutzer können jederzeit ihre persönlichen Informationen einsehen, verändern oder löschen (der Benutzername kann nicht verändert werden). Administratoren der Website können diese Informationen ebenfalls einsehen und verändern.</p>
                            </div>
